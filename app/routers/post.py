...
def get_my_posts(db:Session=Depends(get_db),curr_user:int=Depends(oauth2.get_current_user)):
    posts=db.query(models.Post).filter(curr_user.id==models.Post.owner_id).all() # type: ignore
    return posts


@router.get("/latest",response_model=schemas.PostResponse)
def get_latest_posts(db: Session = Depends(get_db),curr_user:int=Depends(oauth2.get_current_user)):
    post=db.query(models.Post).order_by(models.Post.id.desc()).first()  # type: ignore
    if not post:
        raise HTTPException(
...
@router.put("/{id}", status_code=200,response_model=schemas.PostResponse,)
